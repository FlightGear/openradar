...
public class StdRouteRestriction extends AStdRouteElement {
    ...
    public StdRouteRestriction(StdRoute route, IMapViewerAdapter mapViewAdapter, AStdRouteElement previous,
                        String position, String maxspeed, String notabove, String notbelow, String font, String fontSize,
                        String stroke, String color) {
        super(mapViewAdapter, route.getPoint(position,previous),stroke,null,null,color);
    ...
    public synchronized Rectangle2D paint(Graphics2D g2d, IMapViewerAdapter mapViewAdapter) {
        ...
        int midlineY = (int)(displayPoint.getY());						// y co-ord of the middle line of the object
        																// every graphical component of a restriction
        																// will have it's y co-rd relative to this
        int margin = 0;
        ...
        int widestX = (int) (slot2Bounds.getWidth() > slot4Bounds.getWidth() ? slot2Bounds.getWidth(): slot4Bounds.getWidth());
        ...
        bounds = new Rectangle2D.Double(displayPoint.getX(), displayPoint.getY(),widestX + (2*margin), textHeight + (4*margin) + (4*linewidth));
        ...
        if (slot1Line)		//upper line
        	g2d.drawLine(lineStartX, slot1Y, lineEndX, slot1Y);
        					...
        					//upper text
        g2d.drawString(slot2Text, (int)(displayPoint.getX()-slot2Bounds.getCenterX()), midlineY - (fm.getDescent() + margin + (linewidth/2)));

        if (slot3Line)		//middle line
        	g2d.drawLine(lineStartX, midlineY, lineEndX, midlineY);

        					//lower text
        g2d.drawString(slot4Text, (int)(displayPoint.getX()-slot4Bounds.getCenterX()), (int)(midlineY + slot4Bounds.getHeight() + margin + (linewidth/2)));
    ...
    private int altInFeet (String alt) {
    	if (alt != null) {
    		...
    		alt=alt.replace("GND","0");										// the ground are 0ft is wrong,
    																		...
    																		// restriction there, the workaround is to
    																		// specify the actual height in -ve feet
    		try {
    			return Integer.parseInt(alt);								//plain integer e.g. 3000
    		}
    		catch (NumberFormatException e){
    			try {
    				return Integer.parseInt(alt.replace("ft",""));  		//ft specified e.g. 3000ft
    			}
        		catch (NumberFormatException e1){
        			try {
        				return Integer.parseInt(alt.replace("FL","")) * 100;//Flight Level specified e.g. FL120
        			}
