...
public class StdRouteReader {
	...
	private void readRouteXml() {
		...
		while (files.size() > 0) {
			...
			try {
				...
				StdRouteAttributes FileDefaultAttributes = new StdRouteAttributes();
				...
				StdRouteAttributes RoutesDefaultAttributes = new StdRouteAttributes(FileDefaultAttributes, rootNode);

				// String orFilename = "data/routes/" + data.getAirportCode() +
				// "/" + file.getName().substring(0,
				// file.getName().indexOf(".xml")) + ".or.xml";
				if ("ProceduresDB".equalsIgnoreCase(rootNode.getName())) {// &&
																			// !(new
																			// File(orFilename).exists()))
																			// {
					// if converted file does not exist, convert it now.
					// deactivated for now convertProcedureDbFile(orFilename,
					// rootNode);
				} else {
					// read or file
					...
					for (Element eRoute : list) {
						StdRouteAttributes routeDefaultAttributes = new StdRouteAttributes(RoutesDefaultAttributes,
								eRoute);
						...
						for (Element element : sublist) {
							try {
								StdRouteAttributes attributes = new StdRouteAttributes(routeDefaultAttributes, element);
								if (element.getName().equalsIgnoreCase("activeLandingRunways")) {
									...
									route.setNavaids(element.getText(), attributes);
									...
									StdRouteLine line = new StdRouteLine(data, route, mapViewAdapter, previous, start,
									...
									String arrows = element.getAttributeValue("arrows");
									String text = element.getAttributeValue("text");
									StdRouteBow bow = new StdRouteBow(data, route, mapViewAdapter, previous, center,
											radius, startAngle, extentAngle, arrows, text, attributes);
									previous = bow;
									...
									String arrows = element.getAttributeValue("arrows");
									StdRouteCurve bow = new StdRouteCurve(data, route, mapViewAdapter, previous, start,
											end, controlPoint, arrows, attributes);
									previous = bow;
									route.addElement(bow);
									...
									String text = element.getAttributeValue("text");
									String arrows = element.getAttributeValue("arrows");
									StdRouteIntercept intercept = new StdRouteIntercept(data, route, mapViewAdapter,
											previous, start, startOffset, startHeading, startTurn, radius, speed, end,
											radial, endHeading, direction, endOffset, arrows, text, attributes);
									previous = intercept;
									...
									StdRouteLoop ellipse = new StdRouteLoop(data, route, mapViewAdapter, previous,
											navpoint, inboundHeading, length, width, right, arrows, minHeight,
											maxHeight, misapHeight, attributes);
									previous = ellipse;
									...
									StdRouteMultipointLine line = new StdRouteMultipointLine(data, route,
											mapViewAdapter, previous, points, close, attributes);
									previous = line;
									route.addElement(line);
									...
									StdRouteRestriction restriction = new StdRouteRestriction(data, route, mapViewAdapter,
											previous, position, maxspeed, notabove, notbelow, attributes);
									...
									StdRouteText text = new StdRouteText(data, route, mapViewAdapter, previous,
											position, angle, alignHeading, clickable, sText, attributes);
									previous = text;
									...
									StdRouteMinAltitude minAlt = new StdRouteMinAltitude(data, route, mapViewAdapter,
											previous, position, value, attributes);
									previous = minAlt;
	...
	/**
	 * I have given up this part of the project for now, as there are big
	 * problems to map the navdata made to feed an autopilot to an xml made to
	 * add details to the map.
	 *
	 * http://gitorious.org/fg/flightgear/blobs/blame/
	 * 44e672c25651dd3ef359ba300ddad3a2b47879fd/src/Navaids/route.cxx Normal :
	 * /Sid/Star/_Waypoint defined by coordinates
	 *
	 * Hold: a holding pattern
	 *
	 * DmeIntc Waypoint defined by distance from point (dme) DMEtoIntercept the
	 * distance to 'dme' Hdg_Crs_value the course to the waypoint
	 *
	 * Intc Intercept a radial of next waypoint Hdg_Crs_value original course
	 * RadialtoIntercept radial to intercept
	 *
	 *
	 * @param orFilename
	 * @param rootNode
	...
	//
	// Namespace ns =
	// Namespace.getNamespace("http://tempuri.org/XMLSchema.xsd");
	//
	...
	// String altitudeRestriction =
	// eWaypoint.getChildText("AltitudeRestriction",ns);
	//
	// if(newLine==null) {
	// // Start of route
	// if(Double.parseDouble(lat)!=0 && Double.parseDouble(lon)!=0) {
	// // no position given, sids start at runways => runwayend to first
	// waypoint
	...
	// newLine.setAttribute("start",lastWayPoint);
	//
	// } else if(type.equals("Intc") || type.equals("VorRadialIntc")) { //||
	// type.equals("DmeIntc")
	...
	// intercept.setAttribute("startTurning",lastWayPoint); // this draws a line
	// in dir heading
	//
	...
	// // this is not really correct. The direction must be a tangent to the dme
	// circle in distance
	// // todo
	// intercept.setAttribute("end",lastWayPoint);
	// }
	...
	// boolean right =
	// "right".equalsIgnoreCase(eWaypoint.getChildText("Hld_Turn",ns));
	// String timeOrDist = eWaypoint.getChildText("Hld_Time_or_Dist",ns);
	// String tdValue = eWaypoint.getChildText("Hld_td_value",ns);
	//
	// // <loop navpoint="SEDOR" inboundHeading="192" length="5" arrows="both"
	// minHeight="7000" maxHeight="MAX 12000"/>>
	// if(newLine==null) {
	// // create it
