...
public class FlightStripCellRendererTrad extends JComponent implements ListCellRenderer<GuiRadarContact> {
	...
	@Override
	public Component getListCellRendererComponent(JList<? extends GuiRadarContact> list, GuiRadarContact value,
		...
		synchronized (value) {
			if (!value.isAtc()) {
				...
				lbSquawkCode.setText(
						getSquawkDisplay(value)/* + " "+value.getFrequency() */);
				if (value.isNeglect()) {
			...
			taNormalHeight = value.isAtc() ? taNormalHeight + 8
					: taNormalHeight + (int) lbGroundSpeed.getPreferredSize().getHeight() + 8;
			int taCommentHeight = isAtcCommentEmpty ? taNormalHeight
					: taNormalHeight + (int) taAtcComment.getPreferredSize().getHeight() + 8;
	...
	private String composeComment(GuiRadarContact contact) {
		...
		if (!fpd.getDepartureAirport().isEmpty() || !fpd.getDestinationAirport().isEmpty()) {
			...
			remarks.append(" ");
			String airportDirection = fpd.getDirectiontoDestinationAirport(contact.getCenterGeoCoordinates());
			remarks.append(airportDirection.isEmpty() ? "___" : airportDirection);
			remarks.append(" ");
	...
	private String getSquawkDisplay(GuiRadarContact c) {
		...
		if (c.getAssignedSquawk() == null && c.getTranspSquawkCode() == null)
			return "";
		if (c.getAssignedSquawk() != null && c.getTranspSquawkCode() == null && (null == c.getTranspSquawkCode()
				|| -9999 == c.getTranspSquawkCode() || c.getTranspSquawkCode() == null))
			return "" + c.getAssignedSquawk() + "(standby)";
		if (c.getAssignedSquawk() == null && (null == c.getTranspSquawkCode() || -9999 == c.getTranspSquawkCode()
				|| c.getTranspSquawkCode() == null))
			return "(standby)";
		if (c.getAssignedSquawk() == null && -9999 != c.getTranspSquawkCode())
			return "(" + c.getTranspSquawkCode() + ")";
		if (c.getAssignedSquawk().equals(c.getTranspSquawkCode()))
			return "" + c.getTranspSquawkCode();
