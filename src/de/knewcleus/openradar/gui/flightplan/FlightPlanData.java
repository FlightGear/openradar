...
public class FlightPlanData {
	...
	public enum UpdateStatus {
		CHANGED, TO_BE_TRANSMITTED, IN_TRANSMISSION, SYNCHRONIZED
	}
	...
	public FlightPlanData(AirportData airportData, GuiRadarContact contact, String flightCode, String callSign,
			String owner, String handover, String squawk, String assignedRunway, String assignedAltitude,
			String assignedRoute, String fpStatus, String flags, String type, String aircraft, String trueAirspeed,
			String departureAirport, String departureTime, String cruisingAltitude, String route,
			String destinationAirport, String alternativeDestinationAirports, String estimatedFlightTime,
	...
	public void update(FlightPlanData newFp) {

		synchronized (contact) {
			// lock contact at first to avoid dead locks
			synchronized (this) {
	...
	public synchronized FlightPlanData copy() {
		FlightPlanData c = new FlightPlanData(airportData, contact, flightCode, callsign, owner, handover, squawk,
				assignedRunway, assignedAltitude, assignedRoute, fpStatus, flags, type, aircraft, trueAirspeed,
				departureAirport, departure, cruisingAltitude, route, destinationAirport,
				alternativeDestinationAirports, estimatedFlightTime, estimatedFuelTime, pilotName, "" + soulsOnBoard,
				remarks);
	...
	public synchronized boolean isDeparting() {
		return airportData.getAirportCode().equals(getDepartureAirport())
				&& !airportData.getAirportCode().equals(getDestinationAirport());
	}
		...
		synchronized (contact) {
			...
			synchronized (this) {
				...
				this.departureAirport = (contact.getRadarContactDistanceD() < 10 && contact.getGroundSpeedD() < 5)
						? airportData.getAirportCode() : "";
				this.departure = (contact.getRadarContactDistanceD() < 10 && contact.getGroundSpeedD() < 5)
						? FpTimeUtil.getUTCTimeString4Digits(new Date()) : "";
				this.cruisingAltitude = null;
				this.route = null;
	...
	public void startFromHere(AirportData data) {
		synchronized (contact) {
			// lock contact at first to avoid dead locks
			synchronized (this) {
				//        this.assignedAltitude = null;
	...
	public synchronized String toString() {
		return flightCode + ": " + callsign + " " + departureAirport + "(" + departure + ") > " + destinationAirport
				+ " (" + aircraft + "," + fpStatus + ")";
	}
